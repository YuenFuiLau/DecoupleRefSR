...
class ResBlockDecoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.resbk1 = nn.Sequential(
            EncoderLayer(512, 512, 3),
            EncoderLayer(512, 512, 1, activate=False),
            )
        #self.up1 = ConvLayer(512, 256, 3)
        self.up1 = DecoderLayer(512, 256, 3, upsample=True)

        self.resbk2 = nn.Sequential(
            EncoderLayer(256, 256, 3),
            EncoderLayer(256, 256, 1, activate=False),
            )
        #self.up2 = ConvLayer(256, 128, 3)
        self.up2 = DecoderLayer(256, 128, 3, upsample=True)

        self.resbk3 = nn.Sequential(
            EncoderLayer(128, 128, 3),
            EncoderLayer(128, 128, 1, activate=False),
            )
        #self.up3 = ConvLayer(128, 64, 3)
        self.up3 = DecoderLayer(128, 64, 3, upsample=True)

        self.resbk4 = nn.Sequential(
            EncoderLayer(64, 64, 3),
            EncoderLayer(64, 64, 1, activate=False),
            )
        #self.up4 = ConvLayer(64, 32, 3)
        self.up4 = DecoderLayer(64, 32, 3, upsample=True)

        self.hr_conv = EncoderLayer(32, 32, 1)
    ...
    def forward(self, input):
        ...
        y = self.up1(self.resbk1(input))
        y = self.up2(self.resbk2(y))
        y = self.up3(self.resbk3(y))
        y = self.up4(self.resbk4(y))
