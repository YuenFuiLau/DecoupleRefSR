...
from base_arch import Encoder, Decoder, ResBlockDecoder
from FGDCN import flow_warp, FlowGuidedDeformableAlignment
...
class RefineNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.ext_Fsisr = Encoder()
        self.ext_Fref = Encoder()
        self.feat_ext_sisr = Encoder()
        self.feat_ext_ref = Encoder()

        self.dec_to_Itex = Decoder()
        #self.dec_to_IrefSR = ResBlockDecoder()
        self.dec_to_IrefSR = Decoder()

        self.deformAlign = FlowGuidedDeformableAlignment(
            in_channels=512,
            out_channels=512,
            kernel_size=3,
            padding=1,
        ...
        self.fuse_conv = EqualConv2d(
            in_channel=1024,
            out_channel=512,
            kernel_size=1
        )
        self.lract = nn.LeakyReLU(negative_slope=0.2)

        self.pre_conv_FTex = EqualConv2d(
            in_channel=512,
            out_channel=512,
            kernel_size=1
        )

    def forward(self, Isisr, Iref):

        Fsisr = self.ext_Fsisr(Isisr)
        Fref = self.ext_Fref(Iref)

        feat_Isisr = self.feat_ext_sisr(Isisr)
        feat_ref = self.feat_ext_ref(Iref)

        #compute flow P and conf C
        B, C, H, W = feat_Isisr.shape
        feat_Isisr_flatten = feat_Isisr.permute(0, 2, 3, 1).contiguous()
        magn_feat_Isisr_flatten = torch.sqrt( torch.square(feat_Isisr_flatten).sum(dim=-1, keepdim=True) + 1e-8)
        feat_Isisr_flatten = feat_Isisr_flatten / magn_feat_Isisr_flatten

        feat_ref_flatten = feat_ref.permute(0, 2, 3, 1).contiguous().view(-1, C)
        magn_feat_ref_flatten = torch.sqrt( torch.square(feat_ref_flatten).sum(dim=-1, keepdim=True) + 1e-8)
        feat_ref_flatten = feat_ref_flatten / magn_feat_ref_flatten

        corrlMat = torch.matmul(feat_Isisr_flatten, feat_ref_flatten.permute(1,0).contiguous())
        conf_C, _ = torch.max(corrlMat, dim=-1, keepdim=True) # B, H, W, 1
        conf_C = conf_C.permute(0,3,1,2).contiguous()
        flow_P = torch.argmax(corrlMat, dim=-1,keepdim=True)
        flow_Px = flow_P % W
        flow_Py = torch.div(flow_P, W, rounding_mode="floor")
        flow_P = torch.cat([flow_Px, flow_Py], dim=-1) # B, H, W, 2

        #get aligned feat Ftex 
        aligned_feat_Ftex = flow_warp(Fref, flow_P)

        #deform align
        FDtex = self.deformAlign(Fref, aligned_feat_Ftex, Fsisr, flow_P.permute(0,3,1,2).contiguous())

        #fuse to get Ftex
        FTex = self.lract(self.fuse_conv(torch.cat([aligned_feat_Ftex,FDtex], dim=1)))

        #get Itex
        Itex = self.dec_to_Itex(FTex)
        
        #get IrefSR
        F_refSR = self.pre_conv_FTex(FTex) * conf_C + Fsisr
        IrefSR = self.dec_to_IrefSR(F_refSR)
...
if __name__ == "__main__":
    ...
    device = "cuda"
    net = RefineNetwork().to(device)
    xl = torch.randn(1,3,512,512).to(device)
    xr = torch.randn(1,3,512,512).to(device)
